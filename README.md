...
2. Copy `.env.example` to `apps/api/.env` and `apps/web/.env.local` and adjust `DATABASE_URL` if needed. With no `GROQ_API_KEY` the API uses a deterministic fake LLM.
