...
`schema.d.ts` is generated by `pnpm gen` with the API running locally and is committed.
...
Auth guard: `src/lib/supabase/middleware.ts` refreshes the session and redirects unauthenticated requests to `/sign-in` for all routes except auth pages. API calls happen from client components through `createApi(NEXT_PUBLIC_API_URL, () => supabase.auth.getSession().then(s => s.data.session?.access_token ?? null))`.
...
- Playwright (`e2e/`): runs against `pnpm api` with `LLM_PROVIDER=fake` and Supabase local (`supabase start`) or a test project. Spec `first-run.spec.ts`: sign up → answer all onboarding widgets → confirm → pantry setup adds 5 items → Today shows 4 entries with reasons → swap dinner → feedback up → Shopping shows missing items.
